...
var passwordCheckOld = function () 
{
    ...
    if (stringPassword.length < 8) //use .length because your counting the length of pass
    {
      stringOutput = "Please enter a password with more than 8 characters.";
    }
   else
   {
      for (var i = 0; i < arrayBadPasswords.length; i++) //i=o to start at first bad pass, i < array goes through the whole array of passwords, i++ counting up
      {
         if (stringPassword == arrayBadPasswords[i]) //if stringPassword = array of bad passwords, then . . . 
         {
            stringOutput = "Please enter a password that is not included in the bad password list."; 
            $("password").value = ""; //blanks it out
         }
      }
   }
   if (stringOutput!="Please enter a password with more than 8 characters." && stringOutput!="Please enter a password that is not included in the bad password list.") //if none of these happen with the password input, then . . . 
   {
      stringOutput = "You're password meets the requirements. It is good!"; //execute this message
   }

$("output").value = stringOutput; //DOM
